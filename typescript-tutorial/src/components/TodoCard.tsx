...
interface TodoCardProps {
    todo: Todo,
    handleCardDoubleClick: (todo: Todo) => void,
...
const TodoCard: React.FC<TodoCardProps> = ({
    ...
    return (
        <div
            className='relative py-2 px-5 m-1 min-h-[80px] bg-yellow-200 border-0 rounded-lg flex flex-row'>
            <>
                {!todo.isEditable &&
                    <p
                        onDoubleClick={(e) => handleCardDoubleClick(todo)}
                        className={'break-words whitespace-normal w-[98%] ' + (todo.isDone ? 'line-through text-slate-500' : '')}
                    >
                        {todo?.todo}
                    </p>
                }
